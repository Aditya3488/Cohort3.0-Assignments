...
export default function TechScreen({ items }) {
  ...
  useLayoutEffect(() => {
    ...
    if (!supportsHover) {
      // Touch device: icons AUR labels dono permanently visible —
      // hover-based reveal yahan kaam hi nahi karta, isliye sab kuch
      // seedha final state pe set kar dete hain.
      const ctx = gsap.context(() => {
        gsap.set('.tech-box', { opacity: 1, filter: 'grayscale(0) brightness(1)' })
        gsap.set('.tech-label', { opacity: 1 })
      }, screenRef)
      return () => ctx.revert()
    }

    const ctx = gsap.context(() => {
      const glowX = gsap.quickTo(glowRef.current, 'x', { duration: 0.35, ease: 'power3' })
      const glowY = gsap.quickTo(glowRef.current, 'y', { duration: 0.35, ease: 'power3' })
      ...
      function updateLighting() {
        ...
        iconRefs.current.forEach((icon) => {
          ...
          const t = Math.max(0, 1 - dist / LIGHT_RADIUS)
  ...
  return (
    <div className="flex flex-col items-center">
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              top: -GLOW_RADIUS,
              background:
                'radial-gradient(circle, rgba(255,77,46,0.55) 0%, rgba(255,77,46,0.22) 45%, transparent 70%)',
              filter: 'blur(30px)',
            }}
          />
          ...
          <div className="relative z-20 grid grid-cols-5 md:grid-cols-6 gap-6 place-items-center h-full content-center">
            {items.map((item, i) => (
              ...
              >
                ...
                >
                  <item.icon size={20} className="text-fixed-paper" />
                </div>
                <span className="tech-label text-[10px] text-fixed-muted whitespace-nowrap" style={{ opacity: 0 }}>
                  {item.label}
                </span>
              </div>
            ))}
      ...
      <p className="text-xs text-muted mt-4 hidden md:block">Cursor ko screen pe le jao — tech icons roshan honge</p>
      <p className="text-xs text-muted mt-4 md:hidden">Tech stack I work with</p>
    </div>
  )
}